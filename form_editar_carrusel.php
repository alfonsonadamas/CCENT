...
<body>
    ...
    <div class="sidebar">
        ...
        <div class="galeria">
            ...
            <?php
            $query = "SELECT * FROM galeria WHERE id_imagen = '$id'";
            $select = $con->query($query);
            while ($row = $select->fetch_assoc()) {

            ?>
            <div class="admin_editar_form">
                <form class="editar" action="editar.php?id_imagen=<?php echo $row['id_imagen'] ?>" method="POST" enctype="multipart/form-data">
                    <img src="data:image/jpg;base64, <?php echo base64_encode($row['imagen']) ?>" alt="" height="200px">
                    <input type="file" accept="image/jpeg, image/jpg, image/png, image/webp" required name="imagen">
