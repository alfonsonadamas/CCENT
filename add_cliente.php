...
<header class="navegacion" id="navegacion">
    ...
    <h1>Completa tu registro</h1>
        <p>Para crear tu cuenta, introduce una contraseña y tu correo electronico</p>
        <form action="update_user.php?id=<?php echo $id ?>" class="formulario_cuenta" method="POST">
            ...
            <input type="text" required name="confpsw">
            </div>
        ...
        if ($st == 1) {          
    ?>
            <p class="error">Correo ya registrado, por favor inicie sesión</p>
    <?php
        }
    ?>
<?php
        if ($st == 2) {          
    ?>
            <p class="error">Las contraseñas no coinciden por favor intentelo de nuevo</p>
    <?php
        }
